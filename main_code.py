from src.functions import match_by_xy_and_diff, load_layers, check_crs, filter_out_sinking_point,KD_clustering,plot_clusters
...
def main():
    ...
    sinking_points_df = filter_out_sinking_point(result)

    print(f"Number of sinking points (elev_diff > 0): {len(sinking_points_df)}")

    clustered_data = KD_clustering(result)
